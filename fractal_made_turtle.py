...
def crea_l_system(iterazioni, assioma, regole):
    start_string = assioma
    if iterazioni == 0:
        return assioma
    end_string = ""
    for _ in range(iterazioni):
        end_string = "".join(regole[i] if i in regole else i for i in start_string)
        start_string = end_string

    return end_string


def disegna_l_system(t, instructions, angle, distance):
...
def calc_lunghezza(istruzioni, angle, correction_angle):
...
def disegna(iterazioni, axiom, rules, angle, length=None, size=None, correction_angle=0,
    ...
    inst = crea_l_system(iterazioni, axiom, rules)

    width_, height_, min_x, min_y = calc_lunghezza(inst, angle, correction_angle)
    ...
    t.speed(0)
    t.pensize(size)
    disegna_l_system(t, inst, angle, length)
    t.hideturtle()
